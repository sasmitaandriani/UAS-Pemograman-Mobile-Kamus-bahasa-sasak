...
public class MainActivity extends AppCompatActivity {
    ...
    Adapter adapter;
    List<KamusSasak> daftarTransaksi;
    ...
    private void inisialisasiView() {
        ...
        txUsername.setText(SharePreferencesUtility.getUserName(this));
        txSaldo = findViewById(R.id.tx_saldo);
    }

    private void setupListview() {
        adapter = new Adapter(this, daftarTransaksi);
        lvDaftarTransaksi.setAdapter(adapter);
    }

    private void loadDataTransaksi() {
        daftarTransaksi = SharePreferencesUtility.getAllTransaksi(this);
        double saldo = 0;
    ...
    private void changeUserName() {
        ...
        builder.setPositiveButton("Simpan", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                SharePreferencesUtility.saveUserName(getApplicationContext(),input.getText().toString());
                Toast.makeText(getApplicationContext(),"Nama user berhasil disimpan",Toast.LENGTH_SHORT).show();
                txUsername.setText(SharePreferencesUtility.getUserName(getApplicationContext()));
            }
        });
