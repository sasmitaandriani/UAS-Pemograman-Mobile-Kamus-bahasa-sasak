...
public class Activity extends AppCompatActivity{
    ...
    final String[] tipeTransaksi = {KamusSasak.kataSifat, KamusSasak.kataBenda, KamusSasak.kataKerja};
    ...
    private void simpan() {
        if (isDataValid()) {
            ...
            tr.setArti(tilDeskripsi.getEditText().getText().toString());
            tr.setContoh(tilNilai.getEditText().getText().toString());
            tr.setKata(tilModel.getEditText().getText().toString());
            tr.setJenis(spJenisTransaksi.getSelectedItem().toString());
            tr.setTanggal(tanggalTransaksi);
            SharePreferencesUtility.addTransaksi(this,tr);
